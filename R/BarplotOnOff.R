...
#' @param inputObj an ExpressionSet or SummarizedExperiment object containing
...
BarplotOnOff <- function(inputObj, onoff.df, group.by = c('transition',
                                                             'sample_id')) {
    ...
    .stopIfNotDataFrame(onoff.df, 'onoff.df', fun.main)
    ...
    if (group.by == 'transition') {
        sel <- onoff.df$test %in% pdata$sub_cell_type1
        if (sum(sel) == 0) {
            stop(paste("Test cell type in the score data frame are not defined",
                       "exiting function", fun.main))
        }
    } else {
        sel <- onoff.df$test %in% pdata$sample_id
        if (sum(sel) == 0) {
            stop(paste("Test samples in the score data frame are not defined",
                       "exiting function", fun.main))
        }
    }
    onoff.df <- onoff.df[sel,]
    onoff.df <- onoff.df[order(onoff.df$OnOffScore, decreasing=FALSE),]
    ...
    if (group.by == 'transition') {
        plot.me <-
            data.frame(position=rep(c("Below", "Above"), each=nrow(onoff.df)),
                        markers=c(onoff.df$loss.start.mkrs, onoff.df$gain.target.mkrs),
                        markername=rep(c("loss.start.mkrs", "gain.target.mkrs"),
                                  each=nrow(onoff.df)),
                        tags=c(onoff.df$test, onoff.df$test),
        ...
        plot.me <-
            data.frame(position=rep(c("Below", "Above"), each=nrow(onoff.df)),
                       ...
                       markername=rep(c("loss.start.mkrs", "gain.target.mkrs"),
                                      each=nrow(onoff.df)),
                       tags=c(onoff.df$sample_id, onoff.df$sample_id),
                       stringsAsFactors=FALSE)
    }
    ## Transition names
    transition <- paste(onoff.df$start, onoff.df$target, sep="->")
    ...
    return(list(GroupComparisonsForPlot=onoff.df, OnOffBarplotData=plot.me))
}
